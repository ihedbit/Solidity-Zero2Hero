...
contract WingSwapNFT is IWingSwapNFT, ERC721PausableUpgradeable, OwnableUpgradeable, AccessControlUpgradeable {
  ...
  modifier onlyGovernance() {
    require(hasRole(GOVERNANCE_ROLE, _msgSender()), "WingSwapNFT::onlyGovernance::only GOVERNANCE role");
    _;
  }

  /// @dev only the one having a MINTER_ROLE can continue an execution
  modifier onlyMinter() {
    require(hasRole(MINTER_ROLE, _msgSender()), "WingSwapNFT::onlyMinter::only MINTER role");
    _;
  }

  modifier onlyExistingCategoryId(uint256 _categoryId) {
    require(_categoryIds.current() >= _categoryId, "WingSwapNFT::onlyExistingCategoryId::categoryId not existed");
    _;
  }

  function initialize(string memory _baseURI) public initializer {
    ERC721Upgradeable.__ERC721_init("WING NFT", "LNFT");
  ...
  function tokenURI(uint256 _tokenId)
    ...
    returns (string memory)
  {
    require(_exists(_tokenId), "WingSwapNFT::tokenURI:: token not existed");
    ...
    onlyExistingCategoryId(_newCategoryId)
  {
    ...
    require(
      ...
      "WingSwapNFT::updateTokenCategory::tokenId not found"
    );
    require(_categoryToWingSwapNFTList[_newCategoryId].add(_tokenId), "WingSwapNFT::updateTokenCategory::duplicated tokenId");
  ...
  function mint(
    ...
    require(_categoryToWingSwapNFTList[_categoryId].add(newId), "WingSwapNFT::mint::duplicated tokenId");
    _mint(_to, newId);
  ...
  function _setTokenURI(uint256 _tokenId, string memory _tokenURI) internal virtual override {
    require(_exists(_tokenId), "WingSwapNFT::_setTokenURI::tokenId not found");
  ...
  function mintBatch(
    ...
    require(_size != 0, "WingSwapNFT::mintBatch::size must be granter than zero");
